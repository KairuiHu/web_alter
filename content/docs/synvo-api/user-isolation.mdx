...
- **[Create Directory](/docs/synvo-api/create-directory)** — Create a virtual directory in the specified user’s private storage space. Other users cannot see or access directories created here.
- **[List File](/docs/synvo-api/list-files)** — List only the files and folders owned by the specified user, ensuring complete storage isolation.
- **[Upload File](/docs/synvo-api/upload-file)** — Upload files directly into the specified user’s unique storage scope.
- **[Download File](/docs/synvo-api/download-file)** — Download files by file ID, but only if the file is owned by the specified user.
- **[Update File](/docs/synvo-api/update-file)** — Move or rename files within the isolated environment of the specified user. No cross-user access is possible.
- **[Delete File](/docs/synvo-api/delete-file)** — Remove files exclusively from the specified user’s storage.
- **[Get File Status](/docs/synvo-api/get-file-status)** — Check the processing status of files for the specified user only. Status information is not shared with other users.
- **[Get Context](/docs/synvo-api/query-ai)** — Retrieve AI conversation context and profiling specific to the specified user, maintaining strict data isolation.
